...
func (f *Fake) modTimeFor(name string) time.Time {
	if f.ModTimes == nil {
		f.ModTimes = make(map[string]time.Time)
	}
...
func (f *Fake) Stat(name string) (os.FileInfo, error) {
	...
	if target, ok := f.Symlinks[name]; ok {
		...
		if data, ok := f.Files[target]; ok {
			return fakeFileInfo{name: filepath.Base(name), size: int64(len(data)), mode: f.modeFor(target), id: fakeIdentity(target), hasID: true, modTime: f.modTimeFor(target)}, nil
		}
	...
	if data, ok := f.Files[name]; ok {
		return fakeFileInfo{name: filepath.Base(name), size: int64(len(data)), mode: f.modeFor(name), id: fakeIdentity(name), hasID: true, modTime: f.modTimeFor(name)}, nil
	}
	return nil, &os.PathError{Op: "stat", Path: name, Err: os.ErrNotExist}
}
