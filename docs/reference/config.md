...
| `inject_assigned_skills` | boolean |  |  | InjectAssignedSkills controls whether gc appends an "assigned skills" appendix to the agent's rendered prompt. The appendix lists every skill visible to this agent, partitioned into (assigned-to-you, shared-with-every-agent), so agents sharing a scope-root sink can tell which skills are their specialization vs which are the city-wide set. Pointer tri-state: nil -> inherit: inject when the agent has a vendor sink; *true -> explicitly inject (equivalent to the default); *false -> disable, and let the template render any skill guidance itself. See engdocs/proposals/skill-materialization.md. |
| `attach` | boolean |  |  | Attach controls whether the agent's session supports interactive attachment through the chosen provider (for example, tmux attach). When false, the agent can use a lighter non-attachable runtime. Defaults to true. |
